# ANEXO B
# INVENTARIO INICIAL DE BIENES, ACTIVOS, RECURSOS Y APORTES FUNDACIONALES DE FELIPEGIL360

**Versión: 4.1**

Documento integrante del Sistema Documental del Proyecto.

---

> **Definición del Proyecto:** Para efectos del presente documento, toda referencia a **EL PROYECTO** corresponde al proyecto empresarial histórico denominado **FELIPEGIL360**, iniciado oficialmente el 1 de junio de 2022.

---

# 1. OBJETO

El presente Anexo tiene como finalidad identificar los bienes, activos, recursos, derechos de uso, licencias, aportes y demás elementos puestos a disposición del Proyecto durante su etapa fundacional y desarrollo inicial.

Su propósito consiste en dejar constancia de:

- la identidad de quien realizó cada aporte;
- el origen del activo o recurso;
- su titular jurídico;
- su forma de incorporación al Proyecto;
- su finalidad;
- su fecha aproximada de incorporación;
- su estado;
- su responsable de custodia;
- la existencia de financiación o saldos pendientes;
- los documentos que soportan su propiedad, adquisición o utilización.

La inclusión de un bien, activo o recurso en este Inventario no implica automáticamente su transferencia de propiedad al Proyecto ni a la otra parte.

---

# 2. IDENTIFICACIÓN DE LAS PARTES

## ANDRÉS DAVID MEJÍA HERRERA

Mayor de edad, identificado con cédula de ciudadanía No. **1035425065**, expedida en **Copacabana**.

Para efectos del presente Anexo actúa como:

- Fundador;
- Inversionista Fundacional;
- Director Estratégico;
- titular o aportante de diferentes recursos utilizados durante la etapa inicial.

## LUIS FELIPE GIL BETANCUR

Mayor de edad, identificado con cédula de ciudadanía No. **1026152827**, expedida en **Itagüí**.

Para efectos del presente Anexo actúa como:

- Talento Fundador;
- representante deportivo;
- vocero técnico;
- participante operativo y público del Proyecto.

---

# 3. PRINCIPIOS DE INTERPRETACIÓN

El presente Inventario se regirá por los siguientes principios:

- veracidad;
- trazabilidad;
- conservación histórica;
- diferenciación patrimonial;
- respeto por la titularidad jurídica;
- no transferencia tácita;
- documentación de los aportes;
- protección del Capital Empresarial Fundacional;
- actualización mediante registros adicionales;
- prohibición de eliminación del historial.

La utilización reiterada de un activo personal por parte del Proyecto no modificará por sí sola su propiedad.

La administración, custodia, mantenimiento, reparación, mejora o utilización de un bien tampoco transferirá automáticamente su titularidad.

Todo cambio de propiedad deberá constar mediante documento expreso y verificable.

---

# 4. CLASIFICACIÓN JURÍDICA DE LOS ACTIVOS

Cada elemento deberá clasificarse en una de las siguientes categorías:

## A. Propiedad personal de Andrés

Bienes cuya titularidad jurídica corresponde exclusivamente a ANDRÉS DAVID MEJÍA HERRERA.

## B. Propiedad personal de Luis Felipe

Bienes cuya titularidad jurídica corresponde exclusivamente a LUIS FELIPE GIL BETANCUR.

## C. Propiedad del Proyecto

Bienes adquiridos con recursos propios del Proyecto o transferidos expresamente al Patrimonio Empresarial.

## D. Activo personal cedido temporalmente para uso

Bien personal utilizado por el Proyecto sin transferencia de propiedad.

## E. Activo licenciado

Derecho, contenido, software, marca o activo cuyo uso fue autorizado sin transferencia de titularidad.

## F. Activo financiado o con obligaciones pendientes

Bien cuyo valor no se encuentra totalmente pagado o respecto del cual existen créditos, cuotas, saldos u obligaciones pendientes.

## G. Servicio o trabajo ejecutado

Actividad profesional, técnica, administrativa, creativa o productiva realizada para el Proyecto.

Su registro reconoce el aporte efectuado, pero no convierte automáticamente conocimientos o capacidades personales en propiedad del Proyecto.

---

# 5. FORMAS DE INCORPORACIÓN

Cada activo deberá indicar cómo fue incorporado al Proyecto.

Las formas permitidas incluyen:

- propiedad personal utilizada temporalmente;
- préstamo de uso;
- comodato;
- licencia;
- arrendamiento;
- adquisición con recursos personales;
- adquisición con ingresos del Proyecto;
- adquisición conjunta;
- patrocinio;
- donación;
- desarrollo interno;
- creación conjunta;
- aporte voluntario;
- transferencia expresa;
- financiación pendiente;
- otra forma debidamente descrita.

---

# 6. MATRIZ GENERAL DEL INVENTARIO

Cada registro deberá utilizar, cuando sea posible, la siguiente estructura:

| ID | Activo o recurso | Titular jurídico | Aportante | Forma de incorporación | Fecha | Valor o referencia | Saldo pendiente | Custodio | Estado | Soporte |
|----|------------------|------------------|----------|------------------------|-------|---------------------|-----------------|----------|--------|---------|

El valor registrado podrá corresponder a:

- valor de adquisición;
- valor comercial aproximado;
- valor contable;
- valor asegurado;
- valor pendiente de pago;
- valor por determinar.

El registro de un valor no modifica la titularidad del activo.

---

# 6A. REGISTRO ESPECIAL DE INSTAGRAM PERSONAL

| Activo | Titular jurídico | Situación al inicio | Tratamiento durante FELIPEGIL360 | Tratamiento económico |
|---|---|---|---|---|
| Cuenta personal de Instagram de LUIS FELIPE GIL BETANCUR | LUIS FELIPE GIL BETANCUR | Preexistente al 1 de junio de 2022; aproximadamente 60 publicaciones y 10.000 seguidores, cifras sujetas a prueba | Utilizada dentro de la estrategia de contenido, producción, edición, administración y crecimiento digital del Proyecto | La cuenta no se transfiere; el valor incremental demostrablemente atribuible a FELIPEGIL360 podrá considerarse en valoración, Buy-Out o liquidación |

Las cifras deberán acreditarse mediante analíticas, capturas, archivos históricos, métricas de interacción e informes técnicos.

Cuando existan indicadores de audiencia artificial o inauténtica, estos deberán documentarse mediante herramienta utilizada, fecha, metodología, porcentaje estimado y soportes verificables.

---

# 6B. AUSENCIA DE INVENTARIO EXHAUSTIVO

El presente Inventario no requiere identificar manualmente cada activo menor, accesorio, cuenta, licencia, archivo o gasto para que pueda ser reconocido.

Los elementos omitidos podrán acreditarse mediante facturas, archivos, publicaciones, metadatos, mensajes, fotografías, videos, testimonios, extractos y demás evidencia válida.

La falta de registro no permitirá apropiarse de un bien ajeno ni desconocer un aporte real.

# 7. ACTIVOS PERSONALES DE ANDRÉS UTILIZADOS POR EL PROYECTO

## 7.1. Suzuki Swift GTi

| Campo | Información |
|-------|-------------|
| Activo | Suzuki Swift GTi |
| Titular jurídico | ANDRÉS DAVID MEJÍA HERRERA |
| Origen | Adquirido antes del nacimiento del Proyecto |
| Clasificación | Propiedad personal utilizada por el Proyecto |
| Transferencia de propiedad | No |
| Uso dentro del Proyecto | Competencias, pruebas, producción audiovisual, generación de contenido y posicionamiento |
| Custodio | ANDRÉS DAVID MEJÍA HERRERA |
| Estado | Activo personal operativo sujeto a desgaste, reparaciones y mantenimiento; su condición exacta se verificará al momento de inventario, entrega o valoración |
| Soportes | Documento de propiedad, fotografías, facturas, registros de mantenimiento y demás evidencias disponibles |

El Suzuki Swift GTi continuó siendo propiedad personal de ANDRÉS DAVID MEJÍA HERRERA.

Su utilización en actividades deportivas, audiovisuales, comerciales o promocionales no implicó transferencia de propiedad al Proyecto ni a LUIS FELIPE GIL BETANCUR.

Las inversiones, reparaciones, mejoras, mantenimientos y gastos realizados sobre el vehículo deberán registrarse separadamente, identificando:

- quién efectuó el pago;
- el origen de los recursos;
- la fecha;
- el valor;
- la finalidad;
- el efecto de la mejora;
- el soporte disponible.

La existencia de mejoras financiadas por el Proyecto no modificará automáticamente la propiedad del vehículo, salvo acuerdo escrito en contrario.

---

## 7.2. Equipos audiovisuales de propiedad de Andrés

Deberán incorporarse individualmente los equipos adquiridos o aportados por ANDRÉS DAVID MEJÍA HERRERA, incluyendo cuando corresponda:

- cámaras;
- cámaras de acción;
- drones;
- lentes;
- estabilizadores;
- micrófonos;
- luces;
- baterías;
- memorias;
- trípodes;
- soportes;
- accesorios;
- equipos de transmisión.

Cada equipo deberá identificar:

- marca;
- modelo;
- número de serie;
- fecha de adquisición;
- precio;
- titular;
- forma de pago;
- estado;
- ubicación;
- soporte de compra.

La utilización de estos equipos para producir contenido del Proyecto no transferirá su propiedad, salvo documento expreso.

---

## 7.3. Equipos informáticos y tecnológicos de Andrés

Registrar individualmente:

- computadores;
- computadores portátiles;
- estaciones de edición;
- monitores;
- discos de almacenamiento;
- servidores;
- dispositivos móviles;
- redes;
- licencias;
- almacenamiento en nube;
- infraestructura tecnológica.

Cuando un equipo personal sea utilizado parcialmente para el Proyecto, deberá registrarse como activo personal cedido temporalmente para uso y no como propiedad del Proyecto.

---

## 7.4. Herramientas, infraestructura y otros recursos

Registrar, cuando corresponda:

- herramientas mecánicas;
- espacios de trabajo;
- almacenamiento;
- conexiones;
- servicios tecnológicos;
- suscripciones;
- transporte;
- infraestructura de producción;
- mobiliario;
- demás recursos personales utilizados.

---

# 8. BIENES PERSONALES DE LUIS FELIPE UTILIZADOS POR EL PROYECTO

Deberán registrarse individualmente los bienes personales puestos a disposición del Proyecto por LUIS FELIPE GIL BETANCUR, incluyendo cuando corresponda:

- implementos deportivos;
- casco;
- traje;
- guantes;
- calzado;
- elementos de seguridad;
- herramientas;
- equipos técnicos;
- licencias deportivas;
- certificaciones;
- otros bienes materiales.

La utilización de estos bienes no implica transferencia de propiedad al Proyecto.

Las licencias deportivas y certificaciones personales continuarán perteneciendo a su titular y únicamente podrán utilizarse dentro de los alcances legalmente permitidos.

---

# 9. CONOCIMIENTOS, EXPERIENCIA, HABILIDADES E IMAGEN

Los conocimientos técnicos, experiencia profesional, habilidades deportivas, nombre civil, voz, imagen y demás atributos personales de LUIS FELIPE GIL BETANCUR no constituyen bienes materiales transferidos al Proyecto.

Tampoco se entenderán transferidos los conocimientos, experiencia, capacidades estratégicas, administrativas, productivas, tecnológicas o empresariales de ANDRÉS DAVID MEJÍA HERRERA.

El Proyecto podrá beneficiarse contractualmente de la aplicación de dichos conocimientos, habilidades y atributos dentro de su objeto.

Sin embargo:

- los conocimientos personales no cambian de titular;
- las habilidades personales no son apropiables;
- la imagen y la voz conservan su protección legal;
- la experiencia previa continúa perteneciendo a cada persona;
- el Proyecto tendrá únicamente los derechos de explotación expresamente acordados;
- los productos, documentos, contenidos o activos creados mediante dichas capacidades tendrán la titularidad definida en el Contrato Maestro.

El reconocimiento de una capacidad personal no significa que su valor comercial actual existiera antes del Proyecto.

La marca, comunidad, contenido, posicionamiento, biblioteca audiovisual y goodwill desarrollados durante el Proyecto deberán registrarse separadamente como activos empresariales.

---

# 10. ACTIVOS ADQUIRIDOS DURANTE EL PROYECTO

Todo activo adquirido durante la vigencia del Proyecto deberá identificar:

- fuente de los recursos;
- responsable del pago;
- existencia de financiación;
- porcentaje efectivamente pagado;
- saldo pendiente;
- titular registrado;
- uso autorizado;
- responsable de custodia;
- forma prevista de distribución o reconocimiento.

El hecho de que un activo se utilice principalmente en actividades del Proyecto no determina por sí mismo su titularidad.

Cuando un activo haya sido adquirido mediante aportes de varias fuentes, deberán documentarse:

- porcentaje aportado por cada parte;
- recursos provenientes de patrocinadores;
- ingresos del Proyecto utilizados;
- financiación pendiente;
- obligaciones asociadas;
- acuerdo de titularidad;
- reglas de venta o liquidación.

---

# 11. YACAR CAR CROSS

El vehículo Yacar Car Cross deberá registrarse en la Matriz General mediante información verificable y soportada.

El registro deberá indicar, como mínimo:

- titular jurídico actual;
- porcentaje económico reconocido a cada parte;
- valor total de adquisición;
- pagos realizados;
- origen de cada pago;
- saldo financiado o pendiente;
- acreedor;
- condiciones de financiación;
- responsable de mantenimiento;
- responsable de custodia;
- régimen de venta;
- distribución aplicable en caso de liquidación;
- seguros;
- obligaciones pendientes.

Las referencias históricas aproximadas sobre porcentajes o financiación deberán validarse con comprobantes, contratos, transferencias y demás documentos disponibles antes de su firma definitiva.

Ninguna afirmación contenida en este Anexo reemplazará los documentos de propiedad o financiación del vehículo.

---

# 11.1. Ficha económica y operativa del Yacar Car Cross

| Campo | Información |
|---|---|
| Activo | Yacar Car Cross |
| Titular jurídico registral | No aplica o no determinado mediante registro público |
| Fecha de adquisición | Enero de 2025 |
| Valor total de adquisición | $65.000.000 COP |
| Pagado por Andrés | $30.000.000 COP |
| Origen del aporte de Andrés | Crédito personal asumido en junio de 2025 |
| Pagado por Luis Felipe | $20.000.000 COP |
| Origen del aporte de Luis Felipe | Crédito personal asumido en diciembre de 2025 |
| Total pagado | $50.000.000 COP |
| Saldo pendiente | $15.000.000 COP |
| Aporte de patrocinadores al precio de compra | $0 COP |
| Acreedor registrado en este documento | No aplica |
| Responsabilidad actual frente al saldo | Ambas partes, según los pagos que efectivamente realice cada una |
| Custodio actual | ANDRÉS DAVID MEJÍA HERRERA |
| Seguro | No aplica |
| Eventos realizados | Aproximadamente siete eventos |
| Uso principal | Eventos, competencias, entrenamientos, campañas y producción de contenido |
| Regla de venta | Podrá venderse si las partes deciden no continuar o si su sostenimiento deja de ser conveniente |
| Tratamiento del saldo | Deberá pagarse antes de distribuir el dinero de una venta |

## Participación económica provisional

Sobre los **$50.000.000 COP efectivamente pagados**, la participación económica provisional corresponde a:

| Parte | Valor pagado | Participación sobre lo pagado |
|---|---:|---:|
| ANDRÉS DAVID MEJÍA HERRERA | $30.000.000 | 60 % |
| LUIS FELIPE GIL BETANCUR | $20.000.000 | 40 % |
| Total pagado | $50.000.000 | 100 % |

Estos porcentajes son provisionales.

La participación definitiva se calculará de acuerdo con la siguiente fórmula:

**La participación definitiva de cada parte se calculará dividiendo el total efectivamente pagado por esa parte entre el total efectivamente pagado por ambas partes y multiplicando el resultado por cien.**

La responsabilidad conjunta frente al saldo pendiente no significa que cada parte tenga automáticamente derecho a la mitad de dicho saldo.

La participación se ajustará conforme a los pagos reales que se efectúen.

## Mantenimiento y preparación

Durante el primer año, ANDRÉS DAVID MEJÍA HERRERA asumió gastos relacionados con el mantenimiento y preparación del vehículo.

Entre las actividades identificadas se encuentran:

- preparación general;
- revisión y alineación de frenos;
- labores relacionadas con el sistema de frenos;
- resortes de suspensión;
- puesta a punto;
- preparación previa a eventos.

La existencia de estos costos queda reconocida. Su cuantificación permanece abierta únicamente hasta consolidar soportes y evitar doble contabilización.

En los trabajos de suspensión y llantas se reconoce preliminarmente:

- aproximadamente 35 % cubierto con recursos personales de ANDRÉS DAVID MEJÍA HERRERA;
- valor restante aportado por el patrocinador Interllantas.

El aporte de Interllantas deberá clasificarse como patrocinio y no como aporte personal de Andrés o de Luis Felipe.

## Custodia

La custodia actual corresponde a ANDRÉS DAVID MEJÍA HERRERA.

La custodia:

- no modifica la participación económica;
- no equivale a propiedad exclusiva;
- implica deber razonable de conservación;
- implica informar daños relevantes;
- impide la disposición unilateral del activo.

## Uso y desgaste

El vehículo ha participado aproximadamente en siete eventos deportivos.

El desgaste normal derivado del uso autorizado deberá considerarse costo empresarial y deportivo del activo.

No será atribuido exclusivamente a una parte, salvo que exista evidencia de:

- uso no autorizado;
- dolo;
- negligencia grave;
- incumplimiento probado.

## Venta y distribución

El vehículo podrá ponerse en venta cuando:

- las partes decidan no continuar con la actividad;
- no existan recursos suficientes para sostenerlo;
- exista una oferta empresarialmente razonable;
- su conservación resulte inconveniente;
- se produzca reorganización, terminación o liquidación.

El dinero recibido se aplicará en el siguiente orden:

1. costos directos de la venta;
2. pago de los $15.000.000 COP pendientes o del saldo que exista en ese momento;
3. pago de obligaciones directamente asociadas con el vehículo;
4. gastos indispensables para efectuar la venta;
5. distribución del valor neto según la participación económica definitiva.

Para la distribución podrá analizarse:

- valor comercial;
- depreciación;
- uso deportivo;
- desgaste;
- daños;
- mejoras;
- estado mecánico;
- obligaciones pendientes.

Los gastos de mantenimiento no aumentarán automáticamente el porcentaje de propiedad, salvo acuerdo escrito que los capitalice.

---

# 11.2. Suzuki Swift GTi como patrimonio personal y referencia empresarial

El Suzuki Swift GTi continúa siendo patrimonio personal y exclusivo de ANDRÉS DAVID MEJÍA HERRERA.

Para obtener resultados deportivos, generar contenido y desarrollar oportunidades para EL PROYECTO, Andrés destinó aproximadamente **$40.000.000 COP** a su desarrollo, preparación y evolución.

Esta cifra:

- no constituye deuda a cargo de Luis Felipe;
- no constituye cobro automático contra EL PROYECTO;
- no modifica la propiedad del vehículo;
- no crea copropiedad;
- sirve como evidencia del compromiso, riesgo e intención empresarial;
- no deberá contabilizarse nuevamente cuando comprenda conceptos ya registrados por separado.

Adicionalmente, durante aproximadamente seis eventos se produjeron daños o intervenciones en cajas de transmisión, con un promedio de **$1.200.000 COP por evento**, para un valor estimado de **$7.200.000 COP**.

Este valor solo se registrará separadamente de los $40.000.000 COP cuando corresponda a gastos adicionales y no incluidos dentro de la cifra consolidada de desarrollo.

---

# 11.3. Parqueadero de vehículo y tráiler

Desde el año 2024, ANDRÉS DAVID MEJÍA HERRERA ha asumido aproximadamente **$130.000 COP mensuales** por parqueadero del vehículo y del tráiler utilizados para las actividades de EL PROYECTO.

Para el corte provisional al 20 de julio de 2026 se reconocen treinta y una (31) mensualidades estimadas, equivalentes a **$4.030.000 COP**. El valor podrá ajustarse con los pagos acreditados.

Estos pagos:

- no constituyen donación;
- no transfieren propiedad;
- no crean obligación permanente de continuar pagándolos;
- deberán registrarse como gasto operativo, anticipo o aporte pendiente de reconocimiento, según corresponda.

---

# 11.4. INVENTARIO FÍSICO, AUDIOVISUAL Y TECNOLÓGICO

Las partes dejan constancia de los activos físicos, audiovisuales, tecnológicos, deportivos y de almacenamiento utilizados para el desarrollo de EL PROYECTO.

La utilización de bienes personales dentro de EL PROYECTO no transfiere automáticamente su propiedad.

Los activos identificados como aportados por ANDRÉS DAVID MEJÍA HERRERA continúan siendo de su propiedad personal, salvo transferencia expresa, escrita y documentada.

## 11.4.1. Inventario consolidado

| Activo o recurso | Cantidad | Propietario, aportante o responsable | Modalidad | Valor aproximado | Estado u observaciones |
|---|---:|---|---|---:|---|
| Suzuki Swift GTi | 1 | ANDRÉS DAVID MEJÍA HERRERA | Activo personal utilizado por EL PROYECTO | Desarrollo histórico referencial registrado en otros anexos | Patrimonio personal de Andrés |
| Yacar Car Cross | 1 | Participación económica según pagos efectuados por Andrés y Luis Felipe | Activo destinado a la actividad deportiva | $65.000.000 COP | Sujeto a saldo pendiente y participación definitiva |
| Tráiler | 1 | Propietario externo | Bien recibido en préstamo | No aplica como activo de EL PROYECTO | Debe mantenerse y devolverse en buen estado |
| GoPro 12 | 2 | ANDRÉS DAVID MEJÍA HERRERA | Activos personales aportados en uso | $1.700.000 COP cada una; total $3.400.000 COP | Seminuevas |
| GoPro 13 | 2 | ANDRÉS DAVID MEJÍA HERRERA | Activos personales aportados en uso | $1.900.000 COP cada una; total $3.800.000 COP | Seminuevas |
| Baterías GoPro | 7 | ANDRÉS DAVID MEJÍA HERRERA | Accesorios personales aportados en uso | $120.000 COP cada una; total $840.000 COP | Utilizadas con las cámaras |
| Cargadores GoPro | Los utilizados para las siete baterías | ANDRÉS DAVID MEJÍA HERRERA | Accesorios personales aportados en uso | Incluidos en la valoración conjunta de baterías y accesorios | Uso audiovisual; no se valoran nuevamente por separado |
| Micrófono | 1 | ANDRÉS DAVID MEJÍA HERRERA | Activo personal aportado en uso | $1.000.000 COP | Producción audiovisual |
| Memorias microSD | Varias, capacidad total de 2 TB | ANDRÉS DAVID MEJÍA HERRERA | Almacenamiento removible aportado en uso | $1.500.000 COP total | Grabación y respaldo audiovisual |
| Gimbal | 1 | ANDRÉS DAVID MEJÍA HERRERA | Estabilizador personal aportado en uso | $1.000.000 COP | Producción audiovisual |
| Soportes para cámaras | Varios | ANDRÉS DAVID MEJÍA HERRERA | Accesorios personales aportados en uso | $700.000 COP total | Montaje de cámaras y producción audiovisual |
| Discos duros de 10 TB | 2 | ANDRÉS DAVID MEJÍA HERRERA | Almacenamiento y respaldo | $1.300.000 COP cada uno; total $2.600.000 COP | Capacidad conjunta de 20 TB |
| Discos SSD de 2 TB | 3 | ANDRÉS DAVID MEJÍA HERRERA | Edición y respaldo | $1.000.000 COP cada uno; total $3.000.000 COP | Capacidad conjunta de 6 TB |
| Google Photos | Suscripción mensual | Pagada por ANDRÉS DAVID MEJÍA HERRERA | Almacenamiento y respaldo digital | $50.000 COP mensuales desde 2024 | Estimación provisional al 20 de julio de 2026: 31 meses, $1.550.000 COP |
| CapCut Pro | Suscripción anual | Pagada por ANDRÉS DAVID MEJÍA HERRERA | Edición audiovisual | $300.000 COP anuales desde 2023 | Estimación provisional 2023-2026: 4 anualidades, $1.200.000 COP |
| Lentes aportados | 0 | No aplica | No se registran aportes | $0 COP | No aplica |
| Computadores aportados | 0 | No aplica | No se registran aportes | $0 COP | No aplica |
| Teléfonos aportados | 0 | No aplica | No se registran aportes | $0 COP | No aplica |

## 11.4.2. Valor consolidado de activos personales valorados

| Activo | Cantidad | Valor unitario aproximado | Valor total aproximado |
|---|---:|---:|---:|
| GoPro 12 | 2 | $1.700.000 COP | $3.400.000 COP |
| GoPro 13 | 2 | $1.900.000 COP | $3.800.000 COP |
| Micrófono | 1 | $1.000.000 COP | $1.000.000 COP |
| Baterías GoPro | 7 | $120.000 COP | $840.000 COP |
| Discos duros de 10 TB | 2 | $1.300.000 COP | $2.600.000 COP |
| Discos SSD de 2 TB | 3 | $1.000.000 COP | $3.000.000 COP |
| **Total aproximado valorado** |  |  | **$17.840.000 COP** |

El valor de $17.840.000 COP es una referencia patrimonial de activos personales utilizados por EL PROYECTO.

No constituye automáticamente una deuda personal de Luis Felipe, una cuenta de cobro inmediata, un activo compartido ni una transferencia de propiedad.

## 11.4.3. Cámaras GoPro

ANDRÉS DAVID MEJÍA HERRERA aportó cuatro cámaras GoPro: dos GoPro 12 y dos GoPro 13, todas en estado seminuevo.

Las cámaras continúan siendo propiedad personal de Andrés.

Su utilización no constituye donación, transferencia, copropiedad ni autorización para venderlas o entregarlas a terceros.

No será necesario incluir seriales, fechas individuales, ubicaciones, custodios o fotografías.

## 11.4.3.1. Valor consolidado de cámaras GoPro

| Modelo | Cantidad | Valor unitario aproximado | Valor total aproximado | Estado |
|---|---:|---:|---:|---|
| GoPro 12 | 2 | $1.700.000 COP | $3.400.000 COP | Seminuevas |
| GoPro 13 | 2 | $1.900.000 COP | $3.800.000 COP | Seminuevas |
| **Total** | **4** |  | **$7.200.000 COP** | Seminuevas |

El valor total aproximado de las cuatro cámaras GoPro corresponde a **$7.200.000 COP**.

Este valor constituye una referencia patrimonial de activos personales de ANDRÉS DAVID MEJÍA HERRERA utilizados por EL PROYECTO y no representa automáticamente una deuda a cargo de EL PROYECTO o de LUIS FELIPE GIL BETANCUR.

---
## 11.4.4. Baterías y cargadores

ANDRÉS DAVID MEJÍA HERRERA aportó siete baterías GoPro y sus cargadores.

Cada batería se valora aproximadamente en **$120.000 COP**, para un valor total aproximado de **$840.000 COP**.

Continúan siendo propiedad personal de Andrés.

## 11.4.5. Micrófono

ANDRÉS DAVID MEJÍA HERRERA aportó un micrófono valorado aproximadamente en $1.000.000 COP.

Continúa siendo propiedad personal de Andrés.

## 11.4.6. Infraestructura de almacenamiento

| Tipo | Cantidad | Capacidad individual | Capacidad total | Valor total |
|---|---:|---:|---:|---:|
| Discos duros | 2 | 10 TB | 20 TB | $2.600.000 COP |
| Discos SSD | 3 | 2 TB | 6 TB | $3.000.000 COP |
| **Total** | **5** |  | **26 TB** | **$5.600.000 COP** |

Los discos físicos continúan siendo propiedad personal de Andrés.

La propiedad del medio físico deberá diferenciarse de los derechos intelectuales, de imagen y económicos sobre los archivos almacenados.

## 11.4.7. Memorias microSD

ANDRÉS DAVID MEJÍA HERRERA aportó varias memorias microSD con capacidad total aproximada de 2 TB, valoradas conjuntamente en **$1.500.000 COP**.

Continúan siendo propiedad personal de Andrés y fueron utilizadas para grabación, almacenamiento temporal y respaldo audiovisual.

## 11.4.8. Gimbal

ANDRÉS DAVID MEJÍA HERRERA aportó un gimbal valorado aproximadamente en **$1.000.000 COP**.

Continúa siendo propiedad personal de Andrés y fue utilizado para estabilización y producción audiovisual.

## 11.4.9. Soportes para cámaras

ANDRÉS DAVID MEJÍA HERRERA aportó diversos soportes para cámaras, valorados conjuntamente en **$700.000 COP**.

Continúan siendo propiedad personal de Andrés y fueron utilizados para montaje de cámaras y producción audiovisual.

## 11.4.10. CapCut Pro

Desde 2023, ANDRÉS DAVID MEJÍA HERRERA paga aproximadamente **$300.000 COP anuales** por CapCut Pro para edición y producción de contenido relacionado con EL PROYECTO.

Para el corte provisional se reconocen cuatro (4) anualidades entre 2023 y 2026, equivalentes a **$1.200.000 COP**, sujetas a ajuste según los pagos efectivamente acreditados.

## 11.4.11. Google Photos

Desde 2024, ANDRÉS DAVID MEJÍA HERRERA paga aproximadamente $50.000 COP mensuales por Google Photos para almacenamiento y respaldo de material relacionado con EL PROYECTO.

Para el corte provisional al 20 de julio de 2026 se reconocen treinta y una (31) mensualidades, equivalentes a **$1.550.000 COP**, sujetas a ajuste según los pagos efectivamente acreditados.

## 11.4.12. Tráiler recibido en préstamo

EL PROYECTO utiliza un tráiler recibido en préstamo de un propietario externo.

No pertenece a EL PROYECTO, Andrés ni Luis Felipe. No constituye aporte de capital.

Debe mantenerse y devolverse en buen estado, considerando el desgaste normal del uso autorizado.

No podrá venderse, transferirse, gravarse ni entregarse a terceros sin autorización.

## 11.4.13. Activos no aportados

No se registran lentes, computadores ni teléfonos como aportes patrimoniales.

## 11.4.14. Evidencia

No será necesario adjuntar fotografías, seriales, fechas, ubicaciones, custodios o facturas completas.

La existencia y uso de los activos podrá acreditarse mediante contenido audiovisual, archivos, publicaciones, metadatos, comprobantes, conversaciones, testimonios y otros medios legalmente válidos.

---

# 12. ACTIVOS ADQUIRIDOS CON INGRESOS DEL PROYECTO

Los activos adquiridos exclusivamente con:

- patrocinios;
- monetización;
- publicidad;
- ventas;
- cursos conjuntos creados específicamente para FELIPEGIL360 mediante acuerdo previo y escrito;
- eventos;
- servicios;
- demás ingresos propios;

se presumirán destinados al Patrimonio Empresarial, salvo que:

- el contrato con el patrocinador establezca otra cosa;
- exista acuerdo escrito diferente;
- correspondan a reembolsos;
- correspondan al pago de obligaciones personales previamente documentadas;
- su naturaleza impida legalmente la propiedad empresarial.

La titularidad formal deberá documentarse correctamente.

El simple hecho de utilizar ingresos del Proyecto no subsana la falta de transferencia jurídica cuando la ley exija registro, contrato o documento adicional.

---

# 13. ACTIVOS DIGITALES

Deberán registrarse individualmente:

- Facebook;
- Instagram;
- YouTube;
- TikTok;
- Threads;
- X;
- dominios;
- página web;
- correos institucionales;
- Google Workspace;
- Google Drive;
- Meta Business;
- cuentas publicitarias;
- Google Analytics;
- Google Ads;
- YouTube Studio;
- repositorios;
- almacenamiento;
- herramientas de inteligencia artificial;
- sistemas de automatización.

Cada registro deberá indicar:

- nombre de la cuenta;
- URL o identificador;
- fecha aproximada de creación;
- correo propietario;
- administrador principal;
- administradores secundarios;
- autenticación de dos factores;
- ubicación de códigos de recuperación;
- responsable de respaldo;
- titularidad contractual;
- restricciones de acceso.

El correo utilizado para crear una cuenta no determinará por sí solo su titularidad económica.

---

# 14. ACTIVOS INTANGIBLES

Deberán registrarse separadamente:

- marca;
- nombres comerciales;
- logotipos;
- identidad visual;
- eslóganes;
- biblioteca audiovisual;
- fotografías;
- videos;
- miniaturas;
- guiones;
- cursos conjuntos creados específicamente para FELIPEGIL360 mediante acuerdo previo y escrito;
- manuales;
- metodologías desarrolladas;
- procesos internos;
- automatizaciones;
- prompts;
- asistentes de inteligencia artificial;
- bases de datos;
- CRM;
- goodwill;
- reputación comercial;
- comunidad digital;
- relaciones con patrocinadores;
- estadísticas y analíticas.

Cada activo intangible deberá indicar:

- creador;
- titular jurídico;
- titular de derechos económicos;
- derechos morales aplicables;
- fecha de creación;
- soporte;
- licencia;
- restricciones;
- forma de explotación.

---

# 15. SERVICIOS Y TRABAJO NO REMUNERADO

Los servicios, actividades y tiempo profesional destinados al Proyecto podrán registrarse como aportes históricos.

Entre ellos:

- dirección estratégica;
- administración;
- producción audiovisual;
- edición;
- diseño;
- gestión de redes;
- coordinación logística;
- asesoría técnica;
- preparación deportiva;
- representación pública;
- creación de contenido;
- desarrollo tecnológico;
- gestión comercial.

Su reconocimiento deberá identificar:

- responsable;
- actividad;
- periodo;
- dedicación aproximada;
- finalidad;
- evidencia disponible;
- metodología de valoración, cuando exista.

El registro de trabajo no remunerado no crea automáticamente una deuda salarial ni una relación laboral.

Tampoco deberá utilizarse para imponer obligaciones futuras distintas de las expresamente pactadas en el Contrato Maestro.

---

# 16. FINANCIACIÓN Y APORTES FUTUROS

Los aportes económicos personales realizados por ANDRÉS DAVID MEJÍA HERRERA durante la etapa fundacional deberán registrarse como parte del Capital Empresarial Fundacional cuando cumplan las condiciones del Contrato Maestro y del Anexo C.

Dichos aportes históricos no crean una obligación permanente de continuar financiando el Proyecto.

Las necesidades futuras deberán cubrirse prioritariamente con:

- patrocinios;
- monetización;
- ventas;
- cursos conjuntos creados específicamente para FELIPEGIL360 mediante acuerdo previo y escrito;
- eventos;
- servicios;
- licenciamientos;
- otros ingresos propios del Proyecto.

Todo nuevo aporte personal de ANDRÉS DAVID MEJÍA HERRERA será voluntario y deberá documentarse indicando:

- valor;
- fecha;
- finalidad;
- naturaleza;
- mecanismo de reconocimiento;
- forma de recuperación;
- derechos adicionales, si fueron acordados.

La recuperación total o parcial de aportes históricos no obliga a reinvertir los recursos recuperados.

---

# 17. ESTADO, UBICACIÓN Y CUSTODIA

Cada activo deberá indicar:

- estado físico o funcional;
- ubicación;
- custodio;
- usuario autorizado;
- sistema de respaldo;
- frecuencia de mantenimiento;
- condiciones de devolución;
- riesgos asociados.

Los estados permitidos incluyen:

- nuevo;
- excelente;
- bueno;
- regular;
- averiado;
- en reparación;
- obsoleto;
- retirado;
- vendido;
- destruido;
- transferido.

---

# 18. DOCUMENTOS SOPORTE

Cada registro deberá vincular, cuando exista:

- factura;
- comprobante bancario;
- contrato;
- documento de propiedad;
- certificado;
- licencia;
- garantía;
- fotografía;
- video;
- correo electrónico;
- mensaje;
- comprobante de financiación;
- registro de mantenimiento;
- documento de cesión;
- documento de préstamo;
- cualquier otro medio probatorio idóneo.

La ausencia de factura no elimina automáticamente la existencia del aporte, pero podrá afectar su valoración o fuerza probatoria.

---

# 19. ACTUALIZACIÓN

El presente Inventario permanecerá abierto durante toda la vida del Proyecto.

Las actualizaciones deberán realizarse mediante:

- nuevos registros;
- actas;
- anexos de actualización;
- documentos de transferencia;
- documentos de baja;
- correcciones históricas.

Ningún registro será eliminado.

Cuando exista un error, deberá incorporarse una corrección que conserve:

- el dato original;
- el dato corregido;
- la fecha;
- la justificación;
- el responsable;
- el soporte.

---

# 20. PREVALENCIA DE LA TITULARIDAD JURÍDICA

En caso de controversia sobre un activo, deberán analizarse conjuntamente:

1. el Contrato Maestro;
2. el Anexo H  Matriz de Propiedad;
3. el presente Inventario;
4. los documentos legales de propiedad;
5. los comprobantes de pago;
6. los documentos de financiación;
7. las actas;
8. las demás evidencias disponibles.

Para bienes sujetos a registro, prevalecerá la titularidad legalmente acreditada, sin perjuicio de los derechos económicos, créditos, aportes o compensaciones que puedan corresponder conforme al Contrato Maestro.

---

# 21. DECLARACIÓN FINAL

Las partes reconocen que este documento tiene naturaleza histórica, patrimonial y probatoria.

Su inclusión no transforma automáticamente:

- bienes personales en bienes del Proyecto;
- derechos de uso en derechos de propiedad;
- servicios en activos transferidos;
- conocimientos personales en propiedad empresarial;
- aportes históricos en obligaciones futuras;
- custodia o administración en titularidad.

El Inventario deberá utilizarse para determinar de manera objetiva:

- qué fue aportado;
- por quién;
- bajo qué modalidad;
- con qué recursos;
- durante qué periodo;
- con qué finalidad;
- bajo qué titularidad;
- con qué obligaciones pendientes.

---

# REGISTRO ACTUALIZADO DEL YACAR CAR CROSS

- Documento formal de compraventa: **no existe a la fecha**.
- Persona vendedora: no identificada documentalmente. Su identificación no es requisito para la firma del acuerdo interno y podrá incorporarse posteriormente si existe evidencia.
- Posesión física y custodia actual: **ANDRÉS DAVID MEJÍA HERRERA**.
- Obligación de Andrés: **$30.000.000 COP**, desde junio de 2025, tasa del 1,5 % mensual, pendiente únicamente de reconstruir la base exacta de liquidación, plazo 60 meses.
- Obligación de Felipe: **$20.000.000 COP**, desde diciembre de 2025, tasa del 1,5 % mensual, pendiente únicamente de reconstruir la base exacta de liquidación, plazo 60 meses.
- Saldo con el vendedor: **$15.000.000 COP**, desde julio de 2026, tasa del 1,5 % mensual, pendiente únicamente de reconstruir la base exacta de liquidación.
- Felipe pagará las cuotas de su obligación de $20.000.000 COP y aportará **$200.000 COP mensuales** al saldo de $15.000.000 COP.
- Andrés pagará las cuotas de su obligación de $30.000.000 COP y el valor restante de las cuotas del saldo de $15.000.000 COP.
- La participación económica específica se actualizará según capital efectivamente pagado.
- Intereses, mantenimiento, reparaciones y mejoras se registrarán separadamente.
- No podrá disponerse del vehículo sin autorización escrita de ambas partes.

---

# REGISTRO DE ACTIVOS INTANGIBLES DE CONTINUIDAD

También integran el inventario económico, aunque no sean bienes físicos:

- goodwill;
- reputación incremental;
- posicionamiento;
- comunidad;
- capacidad de convocatoria;
- relaciones;
- valor incremental de cuentas;
- biblioteca y portafolio;
- capacidad de monetización;
- oportunidades derivadas.

Estos activos conservarán su naturaleza empresarial mientras continúen siendo explotados y exista conexión demostrable con FELIPEGIL360.

---

## REGLA DE LA TASA DEL YACAR

La tasa acordada e informada por LAS PARTES es del uno coma cinco por ciento (1,5 %) mensual. La base exacta de liquidación —sobre saldo insoluto, capital inicial u otra fórmula— deberá acreditarse con los comprobantes, mensajes, tablas de pago o demás evidencia disponible. En ausencia de prueba suficiente sobre la base, ninguna parte podrá imponer unilateralmente una fórmula distinta de la que pueda reconstruirse de los pagos efectivamente realizados.
---

## NATURALEZA ARTESANAL Y PRUEBA DEL YACAR

LAS PARTES reconocen que el Yacar Car Cross es un vehículo artesanal de competencia, sin matrícula, licencia de tránsito, factura comercial ordinaria ni contrato formal de compraventa incorporado al Sistema Documental. Esta circunstancia no elimina el acuerdo material de adquisición ni los derechos económicos internos derivados de los pagos. La adquisición, posesión, aportes y saldo podrán acreditarse mediante comprobantes, transferencias, mensajes, fotografías, videos, testimonios, entrega material, historial de pagos y demás medios de prueba legalmente admisibles.

La ausencia de documento formal del vendedor no impide el reconocimiento interno de los pagos ni obliga a suspender la firma del Sistema Documental.


---

# 19A. ACTIVOS COMERCIALES DE PATROCINIO

Se reconocen como activos o recursos comerciales: portafolios, propuestas, contactos, relaciones con patrocinadores, historial de campañas, métricas, casos de éxito, tarifas, metodologías de valoración, capacidad de distribución, comunidad, audiencias, procesos de producción, formatos, reportes, goodwill y recordación comercial.

La imagen de EL TALENTO es un componente esencial, pero no representa por sí sola la totalidad del activo comercial ofrecido.

---

# 19C. INVENTARIO COMERCIAL FUTURO

El inventario deberá incorporar diseños, muestras, materias primas, productos terminados, empaques, etiquetas, códigos SKU, fotografías de producto y mercancía en tránsito.

La propiedad de cada diseño, marca y lote deberá estar documentada.

---

## FORMALIZACIÓN Y COMERCIO

Este documento deberá interpretarse conjuntamente con los Anexos K y L relativos a la constitución de INVERSIONES RACING 360 S.A.S. y a productos, merchandising y comercio electrónico.

# FIRMAS

En constancia de aceptación, las partes firman el presente Anexo.

_________________________________

**ANDRÉS DAVID MEJÍA HERRERA**  
C.C. No. **1035425065**, expedida en **Copacabana**  
Fundador e Inversionista Fundacional  

Firma: ______________________________

Fecha: ______________________________

---

_________________________________

**LUIS FELIPE GIL BETANCUR**  
C.C. No. **1026152827**, expedida en **Itagüí**  
Talento Fundador  

Firma: ______________________________

Fecha: ______________________________
